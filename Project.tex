...
\begin{figure}[H]
	...
	\end{minipage}
\end{figure}
And we can do the same operation in a combination of two signal: $cos(2\pi \dot 3t)+cos(2\pi \dot 5t)$. Then we can get similar result: two peaks at 3 and 5 Hz. Until now, we only consider one dimension.In math, when we talk about 2-D point, we always use complex number to indicate them. We can use $x+iy$ to suggest every points in this figure. And multiply it by a rotating factor $e^{i 2 \pi nt}$, we can get a result:
$$
\mathcal{F}(x,y)=f(t)\dot e^{i 2 \pi ft}
$$
And we can randomly choose N points, and calculate its weight center
$$
\mathcal{W}(f)=\sum\limits_{i=0}^{N}\mathcal{F}(x_i,y_i)=\sum\limits_{i=0}^{N}f(t_i)\dot e^{i 2 \pi ft}
$$
\begin{figure}[H]
