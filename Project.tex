...
\begin{align}
f_T(t)&=C_0+\sum\lim\limits_{n=1}^{N}{[
...
\begin{align}
&\int_{0}^{T}{C_m\mathrm{d}t}=\int_{0}^{T}{(f_T(t) e^{-jm\Omega t}-
	\sum\limits_{ \substack{n=-N \\ n\neq m} }^{N}{C_n e^{j(n-m)\Omega t}})
	\mathrm{d}t} \nonumber \\
...
\begin{align}
	&TC_m=\int_{0}^{T}{f_T(t) e^{-jm\Omega t}} \mathrm{d}t -
		\sum\limits_{ \substack{n=-N \\ n\neq m} }^{N}{
			\int_{0}^{T}C_n e^{j(n-m)\Omega t} \mathrm{d}t
		} \nonumber
\end{align}
...
So, we can get
$$
C_n=\frac{1}{T}\int_0^T{f_T(t) e^{-jn\Omega t} \mathrm{d}t}
$$
\begin{figure}[H]
